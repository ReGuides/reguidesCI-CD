...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. Статистика по сессиям (более точный подсчет уникальных пользователей)
    ...
    addServerLog('info', 'analytics-stats-improved', 'Analytics stats retrieved successfully', { 
      period, 
      pageType, 
      pageId,
      totalViews: stats.total.totalPageViews,
      uniqueVisitors: stats.total.uniqueVisitors,
    ...
    console.error('Analytics stats improved error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    addServerLog('error', 'analytics-stats-improved', 'Error retrieving analytics stats', { error: errorMessage });
