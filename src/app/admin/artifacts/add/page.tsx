  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Артефакт успешно создан:', result);
        alert('Артефакт успешно создан!');
        router.push('/admin/artifacts');
      } else {
        const errorData = await response.json();
        console.error('Ошибка создания артефакта:', errorData);
        alert(`Ошибка создания артефакта: ${errorData.error || 'Неизвестная ошибка'}`);
      }
    } catch (error) {
      console.error('Ошибка при создании артефакта:', error);
      alert('Произошла ошибка при создании артефакта');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-bold text-white">Добавить артефакт</h1>
      </div>
      ...
      <Card className="max-w-2xl bg-neutral-800 border-neutral-700">
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {saving ? 'Сохранение...' : 'Создать артефакт'}
            </Button>
            <Button
              type="button"
              variant="outline"
